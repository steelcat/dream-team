			...
			if(input.hasClass('block-upload__input-main')){
				...
				input.fileupload({
					dataType: 'json',
						...
						$('.block-result__original').css( 'background-image', 'url(upload/resize/' + (data.files[0].name) + ')' );
					}
				});
			} else {
				// Иначе это
				...
				input.fileupload({
					dataType: 'json',
						...
						$('.block-result__watermark').css( 'background-image', 'url(upload/resize/' + (data.files[0].name) + ')' );
					}
				});
			};
		}
